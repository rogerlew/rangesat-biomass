    ...
    return jsonify(
       dict(location=[
            ...
            pasturestats=[
                ...
                'pasturestats/intra-year/<location>/?ranch=<ranch>&pasture=<pasture>&start_year=<start_year>&end_year=<end_year>&start_date=<start_date>&end_date=<end_date>']))
...
def intrayear_pasturestats(location):
    ...
    if agg_func not in ['mean', 'median', 'sum', 'count', 'max', 'min']:
        return jsonify(None)
